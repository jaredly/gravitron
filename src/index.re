...
let stepBullets = (bullets, player, explosions, status) => {
  ...
  List.fold_left(
    ((status, bullets, explosions), bullet) => {
      switch status {
      | Dead(_) => (status, [{...bullet, pos: posAdd(bullet.pos, vecToPos(bullet.vel))}, ...bullets], explosions)
      | Running => {
        let {theta, mag} = vecToward(bullet.pos, player.Player.pos);
        let (status, explosions) = if (mag < bullet.size +. player.Player.size) {
          (Dead(100), [Player.{Explosion.pos: player.pos, size: player.size, color: player.color, timer: 60, totalTime: 60}, ...explosions])
        } else {
          (Running, explosions)
        };
...
let stepExplosions = (explosions) => {
  open Explosion;
  List.fold_left(
    (explosions, {timer} as explosion) => timer > 1 ? [{
      ...explosion,
      timer: timer - 1
...
let draw = ({me, enemies, bullets, explosions, status}, env) => {
  switch status {
  | Dead(0) => newGame
  | _ => {
  ...
  let me = status === Running ? stepMe(me, env) : me;
